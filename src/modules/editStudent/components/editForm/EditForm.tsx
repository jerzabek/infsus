...
export default function EditForm({ student, allStudents, accomodations }) {
  ...
  const _handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    let theRoom = await fetch(
      `http://localhost:3000/api/rooms?roomFloor=${floor}&roomNumber=${room}&accomodationId=${accomodationId}`
    )
      .then(res => res.json())
      .then(res => res.data)
    if (!theRoom) {
  ...
  return (
    <form onSubmit={_handleSubmit} className="row g-3">
      <div className="col-md-6">
        ...
        <br />
        <input data-testid="jmbag-input" id="jmbag" defaultValue={jmbag} onChange={e => setJmbag(e.target.value)} />
        ...
        <br />
        <input width={'100%'} id="faculty" defaultValue={faculty} onChange={e => setFaculty(e.target.value)} />
      </div>
